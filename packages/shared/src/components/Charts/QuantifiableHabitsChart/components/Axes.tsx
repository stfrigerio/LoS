...
const Axes: React.FC<AxesProps> = ({ x, y, width, height, theme, viewType, data }) => {
  ...
  const parseDate = (dateString: string) => {
    ...
    switch (viewType) {
      case 'weekly':
        const weekNumber = d3.timeFormat("%V")(parsedDate);
        const year = parsedDate.getFullYear();
        return d3.timeParse("%Y-W%V-1")(`${year}-W${weekNumber}-1`);
      case 'monthly':
        return new Date(parsedDate.getFullYear(), parsedDate.getMonth(), 1);
      case 'quarterly':
        const quarter = Math.floor(parsedDate.getMonth() / 3);
        return new Date(parsedDate.getFullYear(), quarter * 3, 1);
      default:
        return parsedDate;
    }
  };
